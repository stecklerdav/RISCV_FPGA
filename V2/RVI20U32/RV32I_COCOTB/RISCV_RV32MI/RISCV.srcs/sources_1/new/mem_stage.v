...
module mem_stage (
    ...
    assign mem_fault_tval = mem_q_alu_result;
    ...
    reg [7:0]  debug_selected_byte;

    always @(*) begin
        load_data_aligned  = 32'h0000_0000;
        debug_selected_byte = 8'h00;

        case (mem_q_mem_size)
            SZ_B: begin
                case (mem_q_alu_result[1:0])
                    2'b00: begin
                        debug_selected_byte = mem_resp_rdata[7:0];
                        load_data_aligned = mem_q_mem_unsigned ? {24'b0, mem_resp_rdata[7:0]} :
                                                                  {{24{mem_resp_rdata[7]}}, mem_resp_rdata[7:0]};
                    end

                    2'b01: begin
                        debug_selected_byte = mem_resp_rdata[15:8];
                        load_data_aligned = mem_q_mem_unsigned ? {24'b0, mem_resp_rdata[15:8]} :
                                                                  {{24{mem_resp_rdata[15]}}, mem_resp_rdata[15:8]};
                    end

                    2'b10: begin
                        debug_selected_byte = mem_resp_rdata[23:16];
                        load_data_aligned = mem_q_mem_unsigned ? {24'b0, mem_resp_rdata[23:16]} :
                                                                  {{24{mem_resp_rdata[23]}}, mem_resp_rdata[23:16]};
                    end

                    2'b11: begin
                        debug_selected_byte = mem_resp_rdata[31:24];
                        load_data_aligned = mem_q_mem_unsigned ? {24'b0, mem_resp_rdata[31:24]} :
                                                                  {{24{mem_resp_rdata[31]}}, mem_resp_rdata[31:24]};
                    end
                endcase
            end
            ...
            SZ_H: begin
                if (mem_q_alu_result[1] == 1'b0)
                    load_data_aligned = mem_q_mem_unsigned ? {16'b0, mem_resp_rdata[15:0]} :
                                                              {{16{mem_resp_rdata[15]}}, mem_resp_rdata[15:0]};
                else
                    load_data_aligned = mem_q_mem_unsigned ? {16'b0, mem_resp_rdata[31:16]} :
                                                              {{16{mem_resp_rdata[31]}}, mem_resp_rdata[31:16]};
            end
    ...
    // DEBUG temporal para rv32ui-p-lb
    always @(posedge clk) begin
        if (!rst &&
            mem_resp_valid &&
            mem_q_valid &&
            mem_q_mem_re &&
            mem_q_mem_size == SZ_B) begin

            $display(
                "[LB_DEBUG] time=%0t addr=0x%08x word=0x%08x offs=%0d unsigned=%0d byte=0x%02x result=0x%08x rd=x%0d",
                $time,
                mem_q_alu_result,
                mem_resp_rdata,
                mem_q_alu_result[1:0],
                mem_q_mem_unsigned,
                debug_selected_byte,
                load_data_aligned,
                mem_q_rd
