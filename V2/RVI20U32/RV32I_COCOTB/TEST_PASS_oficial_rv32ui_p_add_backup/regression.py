...
async def regression(dut):
    ...
    if test_name == "TC_CORE_KEY":
        ...
        dut._log.info("Ejecutando 2000 ciclos...")
        dut._log.info("====================================")

        await ClockCycles(dut.clk, 2000)

        pc_current = int(dut.RISCV_COCOTB_i.Core_RV32I_pc.value)

        dut._log.info("====================================")
        dut._log.info(f"FIN TEST OFICIAL: {test_name}")
        dut._log.info("2000 ciclos completados")
        dut._log.info(f"PC_CURRENT = 0x{pc_current:08X}")
        dut._log.info(f"PC_LAST_REDIRECT = 0x{int(dut.pc_debug_last_redirect_0.value):08X}")
        dut._log.info("====================================")

        dut._log.info("TRACE PC - 20 ciclos finales:")
        for i in range(20):
            await ClockCycles(dut.clk, 1)
            pc_trace = int(dut.RISCV_COCOTB_i.Core_RV32I_pc.value)
            dut._log.info(f"TRACE[{i:02d}] PC = 0x{pc_trace:08X}")
