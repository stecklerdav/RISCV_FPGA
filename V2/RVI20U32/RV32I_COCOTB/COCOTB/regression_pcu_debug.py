...
def find_first_signal(dut, names):
    for name in names:
...
def log_pipeline_stall_state(dut, cycle=None):
    ...
    names = [
        ...
        # Entradas reales al pc_unit_0 / control de PC
        ("PCU_PRIV_REDIRECT_VALID", "privileged_pc_redire_0_priv_redirect_valid", "bit"),
        ("PCU_PRIV_REDIRECT_TARGET", "privileged_pc_redire_0_priv_redirect_target", "hex"),

        ("PCU_REDIRECT_VALID_A", "branch_0_pc_redirect_valid", "bit"),
        ("PCU_REDIRECT_TARGET_A", "branch_0_pc_redirect_target", "hex"),
        ("PCU_REDIRECT_VALID_B", "pc_redirect_valid", "bit"),
        ("PCU_REDIRECT_TARGET_B", "pc_redirect_target", "hex"),

        ("PCU_PREDICT_VALID", "branch_predictor_btb_0_pred_valid", "bit"),
        ("PCU_PREDICT_TAKEN", "branch_predictor_btb_0_pred_taken", "bit"),
        ("PCU_PREDICT_NEXT", "branch_predictor_btb_0_pred_next_pc", "hex"),

        ("PCU_DBG_LAST_REDIRECT", "pc_unit_0_pc_debug_last_redirect", "hex"),
        ("PCU_DBG_REDIRECT_PULSE", "pc_unit_0_pc_debug_redirect_pulse", "bit"),
        ("PCU_DBG_LAST_PREDICT", "pc_unit_0_pc_debug_last_predict", "hex"),
        ("PCU_DBG_PREDICT_PULSE", "pc_unit_0_pc_debug_predict_pulse", "bit"),
        ("PCU_DBG_LAST_PRIV_REDIRECT", "pc_unit_0_pc_debug_last_priv_redirect", "hex"),
        ("PCU_DBG_PRIV_REDIRECT_PULSE", "pc_unit_0_pc_debug_priv_redirect_pulse", "bit"),
...
def log_debug_state(dut):
    ...
    signals_32 = [
        ...
        ("PC_DEBUG_LAST_REDIRECT", "pc_unit_0_pc_debug_last_redirect"),
        ("PC_DEBUG_LAST_PRIV_REDIRECT", "pc_unit_0_pc_debug_last_priv_redirect"),
    ...
    signals_1 = [
        ...
        ("PC_DEBUG_REDIRECT_PULSE", "pc_unit_0_pc_debug_redirect_pulse"),
        ("PC_DEBUG_PRIV_REDIRECT_PULSE", "pc_unit_0_pc_debug_priv_redirect_pulse"),
        ("ROM_EN", "ena_1"),
    ]
...
async def wait_tohost(dut, max_cycles=20000):
    ...
    fwd_a = find_first_signal(dut, ["forwarding_0_forward_a", "forward_unit_0_forward_a"])
    fwd_b = find_first_signal(dut, ["forwarding_0_forward_b", "forward_unit_0_forward_b"])

    pc_redirect_valid = find_first_signal(dut, ["branch_0_pc_redirect_valid", "pc_redirect_valid"])
    pc_redirect_target = find_first_signal(dut, ["branch_0_pc_redirect_target", "pc_redirect_target"])
    ex_actual_next_pc = find_first_signal(dut, ["branch_0_ex_actual_next_pc", "ex_actual_next_pc"])

    priv_redirect_valid = find_first_signal(dut, ["privileged_pc_redire_0_priv_redirect_valid", "priv_redirect_valid"])
    priv_redirect_target = find_first_signal(dut, ["privileged_pc_redire_0_priv_redirect_target", "priv_redirect_target"])

    pc_predict_valid = find_first_signal(dut, ["branch_predictor_btb_0_pred_valid", "pc_predict_valid"])
    pc_predict_taken = find_first_signal(dut, ["branch_predictor_btb_0_pred_taken"])
    pc_predict_next = find_first_signal(dut, ["branch_predictor_btb_0_pred_next_pc", "pc_predict_next"])

    pc_debug_last_redirect = find_child_by_name(dut, "pc_unit_0_pc_debug_last_redirect")
    pc_debug_redirect_pulse = find_child_by_name(dut, "pc_unit_0_pc_debug_redirect_pulse")
    pc_debug_last_predict = find_child_by_name(dut, "pc_unit_0_pc_debug_last_predict")
    pc_debug_predict_pulse = find_child_by_name(dut, "pc_unit_0_pc_debug_predict_pulse")
    pc_debug_last_priv_redirect = find_child_by_name(dut, "pc_unit_0_pc_debug_last_priv_redirect")
    pc_debug_priv_redirect_pulse = find_child_by_name(dut, "pc_unit_0_pc_debug_priv_redirect_pulse")
    ...
    for cycle in range(max_cycles):
        ...
        if cycle < 40 or (cycle % 500 == 0) or interesting_pc:
            log_pipeline_stall_state(dut, cycle)

        # Debug específico del loop test_20 en rv32ui-p-add.
        # Queremos comprobar si 0x8000036C (li tp,0) realmente reinicia x4
        # y si el predictor está reinyectando 0x80000370 cuando el branch NO toma.
        if interesting_pc:
            ...
            dut._log.info(
                ...
                f"REDIR_TARGET={fmt_hex_obj(pc_redirect_target)} "
                f"ACTUAL_NEXT={fmt_hex_obj(ex_actual_next_pc)} "
                f"PRIV_REDIR={fmt_dec_obj(priv_redirect_valid)} "
                f"PRIV_TARGET={fmt_hex_obj(priv_redirect_target)} "
                f"PRED_VALID={fmt_dec_obj(pc_predict_valid)} "
                f"PRED_TAKEN={fmt_dec_obj(pc_predict_taken)} "
                f"PRED_NEXT={fmt_hex_obj(pc_predict_next)} "
                f"DBG_LAST_REDIR={fmt_hex_obj(pc_debug_last_redirect)} "
                f"DBG_REDIR_PULSE={fmt_dec_obj(pc_debug_redirect_pulse)} "
                f"DBG_LAST_PRED={fmt_hex_obj(pc_debug_last_predict)} "
                f"DBG_PRED_PULSE={fmt_dec_obj(pc_debug_predict_pulse)} "
                f"DBG_LAST_PRIV={fmt_hex_obj(pc_debug_last_priv_redirect)} "
                f"DBG_PRIV_PULSE={fmt_dec_obj(pc_debug_priv_redirect_pulse)}"
            )
        ...
        if ex_pc_val == 0x80000388:
            ...
            dut._log.info(f"EX_PC = {fmt_hex_obj(ex_pc)}")
            dut._log.info(f"EX_INSTR = {fmt_hex_obj(ex_instr)}")
            dut._log.info(f"FWD_A = {fmt_dec_obj(fwd_a)}")
            dut._log.info(f"FWD_B = {fmt_dec_obj(fwd_b)}")
            dut._log.info(f"PC_REDIRECT_VALID = {fmt_dec_obj(pc_redirect_valid)}")
            dut._log.info(f"PC_REDIRECT_TARGET = {fmt_hex_obj(pc_redirect_target)}")
            dut._log.info(f"EX_ACTUAL_NEXT_PC = {fmt_hex_obj(ex_actual_next_pc)}")
            dut._log.info(f"PRIV_REDIRECT_VALID = {fmt_dec_obj(priv_redirect_valid)}")
            dut._log.info(f"PRIV_REDIRECT_TARGET = {fmt_hex_obj(priv_redirect_target)}")
            dut._log.info(f"PC_PREDICT_VALID = {fmt_dec_obj(pc_predict_valid)}")
            dut._log.info(f"PC_PREDICT_TAKEN = {fmt_dec_obj(pc_predict_taken)}")
            dut._log.info(f"PC_PREDICT_NEXT = {fmt_hex_obj(pc_predict_next)}")
            ...
            dut._log.info(f"BR_RS1_DATA = {fmt_hex_obj(ex_rs1_data)}")
            dut._log.info(f"BR_RS2_DATA = {fmt_hex_obj(ex_rs2_data)}")
            dut._log.info(f"EXMEM_ALU   = {fmt_hex_obj(exmem_alu)}")
            dut._log.info(f"MEM_FWD     = {fmt_hex_obj(mem_fwd)}")
            dut._log.info(f"WB_DATA     = {fmt_hex_obj(wb_data)}")
            ...
            dut._log.info(f"MEM_STAGE_VALID = {fmt_dec_obj(mem_stage_valid)}")
            dut._log.info(f"MEM_STAGE_RD    = {fmt_dec_obj(mem_stage_rd)}")
            dut._log.info(f"MEM_STAGE_RD_WE = {fmt_dec_obj(mem_stage_rd_we)}")
            dut._log.info(f"MEM_STAGE_FWD_DATA = {fmt_hex_obj(mem_fwd_data)}")
