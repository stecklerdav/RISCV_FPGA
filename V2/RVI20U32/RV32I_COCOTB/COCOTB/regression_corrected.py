...
def log_debug_state(dut):
    ...
    for label, name in [
        ...
        ("X3_GP", "regfile_0_debug_x3"),
        ("X4_TP", "regfile_0_debug_x4"),
        ("X5_T0", "regfile_0_debug_x5"),
    ]:
        log_signal(dut, label, find_child_by_name(dut, name))
...
def get_ram_handle(dut):
    ...
    if ram is None:
        raise AssertionError("No encontré la instancia ram_data / ram_data_1")
    return ram
...
async def wait_riscv_official_pass_fail(dut, max_cycles=20000):
    ...
    if gp_sig is None:
        dut._log.warning(
            "No encontré debug_x3/gp. "
            "Solo podré detectar PASS/FAIL por tohost."
        )
    else:
        dut._log.info("Detectando PASS/FAIL oficial también por gp/x3")

    dut._log.info("Esperando PASS/FAIL oficial RISC-V...")
    dut._log.info(
        "TOHOST_ADDRS = " +
    ...
    for cycle in range(max_cycles):
        ...
        if gp_val is not None:
            if gp_val != last_gp:
                dut._log.info(
                    f"[GP_WRITE] cycle={cycle} gp <= 0x{gp_val:08X}"
                )
                last_gp = gp_val
    ...
    if last_pc is not None:
        raise AssertionError(
            f"Timeout esperando PASS/FAIL oficial después de "
            f"{max_cycles} ciclos. "
            f"Último PC observado: 0x{last_pc:08X}, "
            f"último gp={fmt_hex_val(last_gp)}"
        )

    raise AssertionError(
        f"Timeout esperando PASS/FAIL oficial después de {max_cycles} ciclos."
    )
...
async def regression(dut):
    ...
    if test_name == "TC_CSR_USE_NOP":
        await wait_riscv_official_pass_fail(dut, max_cycles=5000)
        return
