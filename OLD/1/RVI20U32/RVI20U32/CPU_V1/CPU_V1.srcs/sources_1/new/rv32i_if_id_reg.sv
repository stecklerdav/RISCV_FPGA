...
module rv32i_if_id_reg (
    ...
    // ------------------------------------------------------------
    // Etapa de alineación:
    // La BRAM entrega instr con 1 ciclo de latencia respecto al PC.
    // Entonces retrasamos pc / pc_plus4 / valid un ciclo para que
    // queden alineados con if_in.instr.
    // ------------------------------------------------------------
    logic [31:0] pc_align_q;
    logic [31:0] pc_plus4_align_q;
    logic        valid_align_q;

    always_ff @(posedge clk) begin
        if (rst) begin
            pc_align_q       <= 32'b0;
            pc_plus4_align_q <= 32'b0;
            valid_align_q    <= 1'b0;
        end
        else if (!hold) begin
            pc_align_q       <= if_in.pc;
            pc_plus4_align_q <= if_in.pc_plus4;
            valid_align_q    <= if_in.valid;
        end
        // else: hold => conservar alineación actual
    end
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            // 4) si no, carga nueva instrucción ya alineada
            if (flush || (!hold && (kill_cnt_q != 0))) begin
                ...
                reg_q.pc       <= pc_align_q;
                reg_q.pc_plus4 <= pc_plus4_align_q;
                reg_q.instr    <= if_in.instr;
                reg_q.valid    <= valid_align_q;
            end
